    ...
    <div className="fixed inset-0 z-70 flex items-center justify-center p-4 animate-fadeIn">
      ...
      >
        <div className="p-6 border-b flex items-center justify-between" style={!isDarkMode ? {borderColor: '#e5e7eb'} : undefined}>
          <h3 className="text-xl font-semibold flex items-center gap-2" style={!isDarkMode ? {color: '#111827'} : undefined}><Users size={22} /> Group Info</h3>
          <button 
            className="p-2 rounded-lg dark:hover:bg-gray-800 transition-colors" 
            onClick={onClose} 
            aria-label="Close"
        ...
        {loading ? (
          ...
          <div className="flex-1 overflow-y-auto scrollbar-hide">
            {/* Header info */}
            <div className="p-6 border-b flex items-center gap-4" style={!isDarkMode ? {borderColor: '#e5e7eb'} : undefined}>
              <div className="w-16 h-16 rounded-full overflow-hidden flex items-center justify-center" style={!isDarkMode ? {backgroundColor: '#e5e7eb'} : undefined}>
                {imageUrl ? (
                  <img src={imageUrl} alt="Group" className="w-full h-full object-cover" />
                ) : (
                  <Users size={28} style={!isDarkMode ? {color: '#374151'} : undefined} />
                )}
              </div>
              <div className="min-w-0">
                <div className="font-semibold text-lg truncate" style={!isDarkMode ? {color: '#111827'} : undefined}>{conv?.settings?.name || conv?.groupName || 'Group Chat'}</div>
                <div className="text-sm" style={!isDarkMode ? {color: '#6b7280'} : undefined}>{(conv?.participants || []).length} members</div>
              </div>
            </div>

            {/* Tabs */}
            <div className="p-6 flex gap-3 border-b border-gray-200 dark:border-gray-800">
              <button className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${tab==='overview'?'bg-green-600 text-white shadow-md':'bg-gray-100 dark:bg-gray-800 text-gray-900 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-700'}`} onClick={()=>setTab('overview')}>Overview</button>
              <button className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${tab==='shared'?'bg-green-600 text-white shadow-md':'bg-gray-100 dark:bg-gray-800 text-gray-900 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-700'}`} onClick={()=>setTab('shared')}>Shared Media</button>
            </div>
            ...
            {tab === 'overview' ? (
              <div className="p-6 space-y-6">
                ...
                <div className="space-y-4">
                  <div className="text-base font-semibold" style={!isDarkMode ? {color: '#111827'} : undefined}>Group settings</div>
                  <div className="space-y-2">
                    <label className="block text-sm font-medium" style={!isDarkMode ? {color: '#111827'} : undefined}>Name</label>
                    <input value={name} onChange={(e)=>setName(e.target.value)} disabled={!isAdmin} className="w-full px-4 py-2 rounded-lg border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 focus:ring-2 focus:ring-green-500 focus:border-transparent disabled:opacity-50 disabled:cursor-not-allowed" style={!isDarkMode ? {backgroundColor: '#ffffff', color: '#111827', borderColor: '#d1d5db'} : undefined} />
                  </div>
                  <div className="space-y-2">
                    <label className="block text-sm font-medium" style={!isDarkMode ? {color: '#111827'} : undefined}>Group Avatar</label>
                    <div className="flex items-center gap-3">
                      <div 
                        className="w-20 h-20 rounded-full overflow-hidden bg-gray-200 dark:bg-gray-700 flex items-center justify-center"
                        style={!isDarkMode ? { backgroundColor: '#e5e7eb' } : undefined}
                      >
                        {imageUrl ? (
                          <img src={imageUrl} alt="Group avatar" className="w-full h-full object-cover" />
                        ) : (
                          <Users size={32} className="text-gray-700 dark:text-gray-400" style={!isDarkMode ? {color: '#374151'} : undefined} />
                        )}
                      </div>
                      <input
                        ref={imageInputRef}
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-sm" style={!isDarkMode ? {color: '#111827'} : undefined}>Require admin approval for invites</span>
                  </div>
                  {isAdmin && (
                ...
                {/* Invite */}
                <div className="space-y-4">
                  <div className="text-base font-semibold" style={!isDarkMode ? {color: '#111827'} : undefined}>Invite member</div>
                  <div className="relative">
                    ...
                    {(searching || loadingFriends) && (
                      <div className="mt-2 text-sm" style={!isDarkMode ? {color: '#111827'} : undefined}>{loadingFriends ? 'Loading friends...' : 'Searching...'}</div>
                    )}
                    {searchResults.length > 0 && (
                      <div className="mt-2 max-h-60 overflow-y-auto scrollbar-hide space-y-2 border rounded-lg p-2" style={!isDarkMode ? {backgroundColor: '#ffffff', borderColor: '#d1d5db'} : undefined}>
                        {searchResults.map(user => (
                          <div key={user.uid} className="flex items-center justify-between p-2 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700" style={!isDarkMode ? {backgroundColor: '#f9fafb'} : undefined}>
                            <div className="flex items-center gap-3 min-w-0">
                              <div className="w-10 h-10 rounded-full overflow-hidden bg-gray-200 dark:bg-gray-700 flex items-center justify-center">
                                {user.photoURL ? (
                                  ...
                                  <UserRound size={20} className="text-gray-700 dark:text-gray-400" />
                                )}
                              </div>
                              <div className="min-w-0">
                                <div className="text-sm font-medium truncate" style={!isDarkMode ? {color: '#111827'} : undefined}>{user.displayName || 'Anonymous'}</div>
                                <div className="text-xs truncate" style={!isDarkMode ? {color: '#6b7280'} : undefined}>{user.email}</div>
                              </div>
                            </div>
                            <button
                    ...
                    {searchQuery.trim().length >= 1 && !searching && searchResults.length === 0 && (
                      <div className="mt-2 text-sm" style={!isDarkMode ? {color: '#6b7280'} : undefined}>No matching friends</div>
                    )}
                  </div>
                  {conv?.pendingInvites && Object.keys(conv.pendingInvites).length>0 && (
                    <div className="mt-4 text-sm">
                      <div className="font-medium mb-3" style={!isDarkMode ? {color: '#111827'} : undefined}>Pending approvals</div>
                      <ul className="space-y-2">
                        {Object.keys(conv.pendingInvites).map(uid=> (
                          <li key={uid} className="flex items-center justify-between p-3 rounded-lg border" style={!isDarkMode ? {backgroundColor: '#f9fafb', borderColor: '#d1d5db'} : undefined}>
                            <div className="flex items-center gap-2" style={!isDarkMode ? {color: '#111827'} : undefined}><UserRound size={16} /><span>{uid}</span></div>
                            {isAdmin && (
                ...
                <div className="space-y-3">
                  <div className="text-base font-semibold" style={!isDarkMode ? {color: '#111827'} : undefined}>Members</div>
                    ...
                    {(conv?.participants || []).map(uid => {
                      ...
                      return (
                        <li key={uid} className="flex items-center justify-between p-3 rounded-lg border" style={!isDarkMode ? {backgroundColor: '#f9fafb', borderColor: '#d1d5db'} : undefined}>
                          <div className="flex items-center gap-3 min-w-0">
                            <div className="w-10 h-10 rounded-full overflow-hidden bg-gray-200 dark:bg-gray-700 flex items-center justify-center">
                              {photo ? <img src={photo} alt={name} className="w-full h-full object-cover"/> : <UserRound size={20} className="text-gray-700 dark:text-gray-400"/>}
                            </div>
                            <div className="min-w-0">
                              <div className="text-sm font-medium truncate" style={!isDarkMode ? {color: '#111827'} : undefined}>
                                {name}
                                {isCurrentUser && <span className="ml-2 text-xs" style={!isDarkMode ? {color: '#6b7280'} : undefined}>(You)</span>}
                              </div>
                              <div className="text-xs flex items-center gap-1" style={!isDarkMode ? {color: '#6b7280'} : undefined}>{role==='admin'?<Shield size={12}/>:null}{role}</div>
                            </div>
                          </div>
                          {isAdmin && !isCurrentUser && (
                            <div className="flex items-center gap-2">
                              <select className="text-sm border rounded-lg px-3 py-1 focus:ring-2 focus:ring-green-500" value={role} onChange={(e)=>handleRole(uid, e.target.value)} style={!isDarkMode ? {backgroundColor: '#ffffff', color: '#111827', borderColor: '#d1d5db'} : undefined}>
                ...
                <div className="pt-4 border-t border-gray-200 dark:border-gray-800">
                  ...
                  {!canLeaveGroup && (
                    <p className="mt-2 text-xs text-center" style={!isDarkMode ? {color: '#6b7280'} : undefined}>
              ...
              <div className="p-6 space-y-6">
                ...
                {media.images.length>0 && (
                  <div>
                    <div className="text-base font-semibold mb-3 flex items-center gap-2" style={!isDarkMode ? {color: '#111827'} : undefined}><ImageIcon size={18}/>Images</div>
                ...
                {media.audios.length>0 && (
                  <div>
                    <div className="text-base font-semibold mb-3 flex items-center gap-2" style={!isDarkMode ? {color: '#111827'} : undefined}><Music2 size={18}/>Audio</div>
                    <div className="space-y-2">
                      {media.audios.map(a => (
                ...
                {media.campaigns.length>0 && (
                  <div>
                    <div className="text-base font-semibold mb-3" style={!isDarkMode ? {color: '#111827'} : undefined}>Campaigns</div>
                    <div className="space-y-3">
                ...
                {media.links.length>0 && (
                  <div>
                    <div className="text-base font-semibold mb-3 flex items-center gap-2" style={!isDarkMode ? {color: '#111827'} : undefined}><LinkIcon size={18}/>Links</div>
                    <ul className="space-y-2 text-sm">
                      {media.links.map(l => (
                        <li key={l.id} className="p-2 rounded-lg" style={!isDarkMode ? {backgroundColor: '#f9fafb'} : undefined}><a className="text-blue-600 dark:text-blue-400 hover:underline break-all" href={l.url} target="_blank" rel="noreferrer">{l.url}</a></li>
                      ))}
                    </ul>
                  </div>
                )}
                ...
                {media.images.length===0 && media.audios.length===0 && media.campaigns.length===0 && media.links.length===0 && (
                  <div className="text-sm text-center py-8" style={!isDarkMode ? {color: '#6b7280'} : undefined}>No shared media yet.</div>
                )}
              </div>
            )}
          </div>
